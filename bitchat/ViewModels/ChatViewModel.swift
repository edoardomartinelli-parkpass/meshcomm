...
final class ChatViewModel: ObservableObject, BitchatDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    /// SOS pins extracted from `[SOS]` broadcasts (sent or received).
    @Published var sosPins: [SOSPin] = []
    ...
    /// Send an SOS broadcast over the BLE mesh with optional GPS coordinates.
    /// Always emits via the local mesh transport regardless of `activeChannel`
    /// so the alert reaches every reachable peer offline. Receivers detect the
    /// `[SOS]` prefix and render the message with emergency styling.
    @MainActor
    func sendSOSMessage(latitude: Double?, longitude: Double?) {
        let nick = self.nickname.isEmpty ? "anon" : self.nickname
        let locationPart: String
        if let lat = latitude, let lng = longitude {
            locationPart = String(format: " @%.4f,%.4f", lat, lng)
        } else {
    ...
    /// Parse a `@lat,lng` token from an SOS broadcast and add a pin if
    /// coordinates are present. Idempotent on `messageID`.
    @MainActor
    func appendSOSPinIfPresent(messageID: String, sender: String, content: String, timestamp: Date) {
        guard content.hasPrefix("[SOS]") else { return }
        guard let coord = Self.extractSOSCoordinate(from: content) else { return }
        let pin = SOSPin(
            id: messageID,
            nickname: sender,
            ...
            timestamp: timestamp,
            isOwn: sender == self.nickname
        )
        if !sosPins.contains(where: { $0.id == pin.id }) {
            sosPins.append(pin)
        }
    }

    static func extractSOSCoordinate(from text: String) -> CLLocationCoordinate2D? {
        ...
        guard let match = regex.firstMatch(in: text, range: range),
              ...
              let lng = Double(text[lngRange]) else { return nil }
    ...
    func handlePublicMessage(_ message: BitchatMessage) {
        ...
        // meshcomm extension: harvest GPS coordinates from `[SOS]` broadcasts
        // before any further routing so the SOS map reflects every alert.
        appendSOSPinIfPresent(messageID: finalMessage.id,
...
// MARK: - SOS Pin (meshcomm)

struct SOSPin: Identifiable, Hashable {
    let id: String
    let nickname: String
    let latitude: Double
